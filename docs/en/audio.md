...
Optional SFX + music + 2D spatial audio via the `Audio` service (raylib/pyray backend).
...
| API | Behavior |
| --- | --- |
| `set_listener(node)` | Auto-follows `node.transform.world.position` from `on_start_frame` |
| `set_listener(x, y)` | Fixed position; clears node follow |
| `clear_listener_follow()` | Stop following; keep the last `(x, y)` |
...
A bank's `base_path` is relative to `AudioConfig.assets_path`; resolved paths
must still stay under that directory.
...
| destroy | Stops the owned voice / music if still the owner |
...
- One global listener (`set_listener` node **or** x/y; no multi-ear)
